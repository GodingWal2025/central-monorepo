...
import { Home, Users, BookOpen, GraduationCap, BookUser, Building2, X, Wrench } from "lucide-react";
...
const NavItem = ({ icon: Icon, label, active, onClick, badge }: NavItemProps) => (
  <button
    onClick={onClick}
    className={`w-full flex items-center gap-3 px-3 py-3 md:py-2.5 rounded-xl text-sm font-medium transition ${active ? 'bg-stone-900 text-white shadow-sm' : 'text-stone-600 hover:bg-stone-100 active:bg-stone-200 hover:text-stone-900'}`}>
    <Icon size={16} />
